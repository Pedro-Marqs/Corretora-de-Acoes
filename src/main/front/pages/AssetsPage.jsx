...
export default function AssetsPage() {
  ...
  const [ticker, setTicker] = useState(''); const [searchedTicker, setSearchedTicker] = useState(''); const [fieldError, setFieldError] = useState('')
  const [state, setState] = useState({ status: 'initial', asset: null, message: '' })
  async function submit(event) {
    event?.preventDefault(); if (lock.current) return
    const normalized = ticker.trim().toUpperCase()
    if (!/^[A-Z0-9]{1,12}$/.test(normalized)) { setFieldError('Informe um ticker com 1 a 12 letras ou números.'); return }
    lock.current = true; setTicker(normalized); setSearchedTicker(normalized); setFieldError(''); setState({ status: 'loading', asset: null, message: '' })
    try {
      const asset = await searchAsset(normalized); setState({ status: asset ? 'success' : 'empty', asset, message: '' })
    } catch (error) {
  ...
  return <main className="assets-page"><header className="assets-heading"><p className="eyebrow">Dados de mercado</p><h1>Pesquisa de ativos</h1><p>Consulte cotações oficiais da simulação pelo ticker.</p></header>
    <section className="asset-search-card" aria-labelledby="asset-search-title"><div><h2 id="asset-search-title">Encontre um ativo</h2><p>Informe somente o ticker, como PETR4 ou AAPL.</p></div><form onSubmit={submit} noValidate><div className="form-field"><label htmlFor="assetTicker">Ticker <span aria-hidden="true">*</span></label><input id="assetTicker" name="ticker" autoComplete="off" maxLength="12" required value={ticker} onChange={(event) => { setTicker(event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '')); setFieldError('') }} aria-invalid={Boolean(fieldError)} aria-describedby={fieldError ? 'assetTicker-error' : 'assetTicker-hint'} placeholder="PETR4" />{fieldError ? <span id="assetTicker-error" className="field-error">{fieldError}</span> : <span id="assetTicker-hint" className="field-hint">Use de 1 a 12 letras ou números.</span>}</div><button className="primary-button" type="submit" disabled={state.status === 'loading'}>{state.status === 'loading' ? 'Pesquisando…' : 'Pesquisar ativo'}</button></form></section>
