...
public class PurchaseTransactionService {
    ...
    public PurchaseResult purchase(UUID accountId, UUID brokerAssociationId, long quantity,
        ...
        Position position = positions.findForUpdateByAccountIdAndAccountBrokerIdAndAssetId(
                accountId, brokerAssociationId, asset.getId()).orElse(null);
        PositionBalance current = position == null
                ? PositionBalance.empty() : position.financialBalance();
        PositionQuantity purchased = PositionQuantity.positive(quantity);
        FinancialAmount unitPriceBrl = new FinancialAmount(marketData.unitPriceBrl());
        PositionBalance resulting = PositionFinancialCalculator.buy(current, purchased, unitPriceBrl);
        FinancialAmount purchaseAmount = unitPriceBrl.multiply(quantity);
        FinancialAmount available = new FinancialAmount(account.getBalance());
        if (purchaseAmount.value().compareTo(available.value()) > 0) {
            throw BusinessRuleException.insufficientBalance(purchaseAmount, available);
        }
        account.debit(purchaseAmount);
        if (position == null) {
            position = positions.save(Position.create(UUID.randomUUID(), account, association,
                    asset, resulting));
        } else {
        ...
        history.record(account, occurredAt, (id, owner, instant) -> Movement.purchase(
                ...
                association.getBroker().getTradeName(), account.getBalance(), instant));
